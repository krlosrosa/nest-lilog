...
export async function createCenterCommand(
  ...
  return await db.transaction(async (tx) => {
    ...
    const existingCenter = await tx.query.center.findFirst({
    ...
    const [newCenter] = await tx.insert(center).values(centerDto).returning();
    ...
    if (existingUserCenter) {
      ...
      // ✅ Cria
      await tx.insert(userCenter).values({
        userId: userId,
